...
//Add Member needs username, email, teamname, category, action, membertitle, memberstatus, lat, long, memberusername, memberemail

//Add Admin needs username, email, teamname, category, action
...
$GoSearch = SearchItemName($manage_array, $MemberID);
...
$GoSearchIndex = SearchItemNameindex($manage_array, $MemberID);
...
echo '{"success":1,"error_message":"'.$MemberID.' has been removed"}';
...
$stmt99 = $dbh->prepare("UPDATE TeamInfo SET ".$itemTypeName." = '".$NewFriendsEncodeDelete."' WHERE teamName = '".$TeamName."'");
$stmt99->execute();
echo '{"success":1,"error_message":"'.$MemberID.': Ammo = '.$ammoCount.'"}';
exit;
...
$stmt99 = $dbh->prepare("UPDATE TeamInfo SET ".$itemTypeName." = '".$NewFriendsEncodeDelete."' WHERE teamName = '".$TeamName."'");
$stmt99->execute();
echo '{"success":1,"error_message":"'.$MemberID.': Level = '.$level.'"}';
exit;
...
$stmt99 = $dbh->prepare("UPDATE TeamInfo SET ".$itemTypeName." = '".$NewFriendsEncodeDelete."' WHERE teamName = '".$TeamName."'");
$stmt99->execute();
echo '{"success":1,"error_message":"'.$MemberID.'":"count":"'.$count.'"}';
