...
$GoSearch = SearchItemName($manage_array, $TargetEmail);
...
$GoSearchIndex = SearchItemNameindex($manage_array, $TargetEmail);
...
$stmtT = $dbh->prepare("Select ".$itemTypeNameTwo." FROM users WHERE email = \"".$TargetEmail."\"");
$stmtT->execute();
$rowT = $stmtT->fetch(PDO::FETCH_ASSOC);
$FriendsDataTwo = $rowT[$itemTypeNameTwo];
$manage_arrayTwo = get_object_vars(json_decode($FriendsDataTwo));
...
$GoSearchIndexTwo = SearchItemNameindex($manage_arrayTwo, $email);

$manageTwo = (array)json_decode($FriendsDataTwo, true);

unset($manageTwo['data'][$GoSearchIndexTwo]);

$NewFriendsEncodeDeleteTwo = json_encode($manageTwo);

//Echo "new array = ".$NewFriendsEncode;

$stmt999 = $dbh->prepare("UPDATE users SET ".$itemTypeNameTwo." = '".$NewFriendsEncodeDeleteTwo."' WHERE email = '".$TargetEmail."'");
$stmt999->execute();
...
echo '{"success":1,"error_message":"'.$TargetEmail.' has been removed"}';
...
$AllMissionInfo[] = Array('location' => $LocationInfo, 'status' => $Status, 'level' => $Level, 'objective' => $Objective, 'xp' => $XP, 'imageURL' => $imageURL, 'objectURL' => $objectURL, 'textureURL' => $textureURL, 'targetname' => $TargetName, 'targetID' => $TargetEmail, 'lastdate' => $time);
...
//echo $itemTypeNameTwo;

//UPDATE THE PERSON YOU ARE TARGETTING'S 'ISTARGETTING' LIST
...
$LocationInfoTwo[] = Array('longitude' => $Long, 'latitude' => $Lat);
...
$AllMissionInfoTwo[] = Array('location' => $LocationInfoTwo, 'targetname' => $username, 'targetID' => $email);


$manageTwo['data'][] = Array(
    'id' => $email,
    'info' => $AllMissionInfoTwo
);



$NewFriendsEncodeTwo = json_encode($manageTwo);

$stmt5 = $dbh->prepare("UPDATE users SET ".$itemTypeNameTwo." = '".$NewFriendsEncodeTwo."' WHERE email = '".$TargetEmail."'");
